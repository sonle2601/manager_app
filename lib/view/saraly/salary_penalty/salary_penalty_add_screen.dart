...
class SalaryPenaltyAddScreen extends StatefulWidget {
  const SalaryPenaltyAddScreen({super.key});

  @override
  State<SalaryPenaltyAddScreen> createState() => _SalaryPenaltyAddScreenState();
}

class _SalaryPenaltyAddScreenState extends State<SalaryPenaltyAddScreen> {
    ...
    return Scaffold(
      ...
      body: Container(
        child: Column(
          children: [
                ...
                onTap: (){
                  SelectEmployeeAccept.showSelectEmployee(context, (selectedEmployeeId, nameEmployee) {
                    setState(() {
                      ...
                      salaryPenaltyVM.employeeId = selectedEmployeeId.toString();
                    });
                  });
                },
            ...
            SizedBox(height: 10,),
            AppTextArea(
              textEditingController: salaryPenaltyVM.amountController.value,
              hintText: "Số tiền",
              keyboardType: TextInputType.number,
            ),
            SizedBox(height: 10,),
            AppTextArea(
              textEditingController: salaryPenaltyVM.reasonController.value,
              hintText: "Lý do",
              maxLines: 3,
            ),
          ],
      ...
      bottomNavigationBar: Obx(() =>
          Container(
            ...
            child: ButtonButtom(
              loading: salaryPenaltyVM.loading.value,
              title: "Thêm mới",
              onPress: () {
                String? validationError = salaryPenaltyVM.validate();
                if (validationError != null) {
                  Utils.snackBar("Lỗi", validationError.toString());
                } else {
                  salaryPenaltyVM.addSalaryPenalty();
                  Get.back();
                }
              },
            ),
